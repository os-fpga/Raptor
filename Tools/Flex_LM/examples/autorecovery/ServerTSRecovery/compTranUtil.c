...
/*
	...
	* View Enterprise License Server fulfillment records.
	...
	Source files
		...
		serverTSRecovery.c				Perform automatic recovery of Server TS
...
int wmain(int argc, wchar_t * wargv[])
{
	...
	/* Check if it is called through ServerTSRecovery() callback function */ 
	if ( !strcmp(sAllocConvertWCToUTF8(wargv[1]), "ServRecovery") )
	{
		servTSRecovery(sAllocConvertWCToUTF8(wargv[2]));
		return 0;
	}
...
{	
    /* Check if it is called through ServerTSRecovery() callback function */ 
	if ( !strcmp(argv[1], "ServRecovery"))
	{
		servTSRecovery(argv[2]);
		return 0;
	}
